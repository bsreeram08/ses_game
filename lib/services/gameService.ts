...
export class GameService {
  ...
  async findGameByInviteCode(inviteCode: string): Promise<string | null> {
    try {
      ...
      const inviteCodeDoc = await getDoc(doc(db, 'inviteCodes', normalizedCode));
      
      if (inviteCodeDoc.exists()) {
        const data = inviteCodeDoc.data();
        if (data && data.gameId) {
          // Verify the game still exists
          const gameDoc = await getDoc(doc(db, this.gamesCollection, data.gameId));
          if (gameDoc.exists()) {
            const game = gameDoc.data() as Game;
            // Only return active games (lobby or playing)
            if (game.status === GameStatus.LOBBY || game.status === GameStatus.PLAYING) {
              return data.gameId;
            }
          }
        }
   ...
   * @returns Response indicating success or failure
   */
  async setPlayerReady(gameId: string, userId: string, ready: boolean): Promise<GameResponse> {
    try {
      // Reference to the game document
      ...
      await updateDoc(gameRef, {
        [`players.${userId}.status`]: ready ? PlayerStatus.READY : PlayerStatus.JOINED,
  ...
  async startGame(gameId: string, hostId: string): Promise<GameResponse> {
    try {
      ...
      return await runTransaction(db, async (transaction) => {
        ...
        const allNonHostReady = nonHostPlayers.every(p => p.status === PlayerStatus.READY);
   ...
   * @returns Response indicating success or failure
   */
  async kickPlayer(gameId: string, hostId: string, playerIdToKick: string): Promise<GameResponse> {
    try {
      // Reference to the game document
      const gameRef = doc(db, this.gamesCollection, gameId);
